...
cols = ['initial', 'date', 'standard', 'gradient', 'injection,ng/fm', 'protein', 'peptides', 'PSMs', 'MS2', 'sequence coverage for BSA', 'TIC intensity']

df1 = pd.read_excel('\\\\amer.pfizer.com\\pfizerfiles\\Research\\LAJ\\oru-tcb\\ChemBio\\Standards on Lumos and Eclipse.xlsx', 'Lumos')
df2 = pd.read_excel('\\\\amer.pfizer.com\\pfizerfiles\\Research\\LAJ\\oru-tcb\\ChemBio\\Standards on Lumos and Eclipse.xlsx', 'Eclipse')
...
if ms_standard.lower() == 'bsa' and t_col_coverage:
    ...
    gradient = '0.5h'
    injection = '60fm'
    additive = pd.Series([user, date, 'BSA', gradient, injection, '', '', '', '', bsa_cov, ''], index=cols)
    t_col_coverage = True
    ...
    additive = pd.Series([user, date, 'Hela', gradient, injection, prot, pep, psm, ms2, '', ''], index=cols)
    t_col_coverage = True
